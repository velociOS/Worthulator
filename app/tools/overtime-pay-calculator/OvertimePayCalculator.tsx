      ...
      <div className="flex flex-col gap-4">
        ...
        >
          ...
          >
            {changeAmount !== 0 && (
              ...
              >
                {changeAmount > 0 ? "+" : ""}{fmt(Math.abs(changeAmount))}{" / wk"}
              </span>
            )}
          </div>
          ...
          <div className="relative mt-2 flex flex-wrap items-center gap-2.5">
            <p className="text-sm font-medium text-gray-400">
              per week &nbsp;&middot;&nbsp;{" "}
              <span className="font-bold text-white">{fmt(annualPay)}</span> / year
            </p>
            {hasOvertime && (
          ...
          {hourlyRate > 0 && (
            <p className="relative mt-2 text-sm font-medium text-emerald-400">
              ...
              <span className="font-bold text-emerald-300">${hourlyRate}/hour</span> earns you
              approximately <span className="font-bold text-emerald-300">{fmt(weeklyPay)}</span> per
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl">
            ...
            <p className="mt-2 text-xl font-bold tracking-[-0.03em] text-gray-900">{fmt(regularPay)}</p>
            <p className="mt-0.5 text-xs text-gray-400">{regularHours}h × ${hourlyRate}</p>
          </div>
          ...
          >
            ...
            <p className={`mt-0.5 text-xs ${hasOvertime ? "text-amber-500" : "text-gray-400"}`}>
              {overtimeHours}h × ${hourlyRate} × {multiplier}×
            </p>
          </div>
          ...
          <div className="col-span-2 rounded-2xl border border-white/6 bg-gray-900 p-5 shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl sm:col-span-1">
            ...
            <p className="mt-2 text-xl font-bold tracking-[-0.03em] text-emerald-400">{fmt(weeklyPay)}</p>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-2xl border border-white/6 bg-gray-900 p-4 shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl">
            ...
            <p className="mt-2 text-xl font-bold tracking-[-0.03em] text-emerald-400">{fmt(dailyPay)}</p>
          ...
          <div className="rounded-2xl border border-white/6 bg-gray-900 p-4 shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl">
            ...
            <p className="mt-2 text-xl font-bold tracking-[-0.03em] text-emerald-400">{fmt(monthlyPay)}</p>
          ...
          <div className="rounded-2xl border border-white/6 bg-gray-900 p-4 shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl">
            ...
            <p className="mt-2 text-xl font-bold tracking-[-0.03em] text-emerald-400">{fmt(annualPay)}</p>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl">
          ...
          <div className="mt-5 rounded-xl border border-white/7 bg-gray-950 px-5 py-4">
            ...
            >
              {fmt(weeklyPay)}<span className="ml-2 text-sm font-normal text-gray-500">/ week</span>
            </p>
            <p className="mt-0.5 text-sm text-gray-400">
              {fmt(monthlyPay)} / mo &nbsp;&middot;&nbsp; {fmt(annualPay)} / yr
            </p>
            {hasOvertime && (
              <p className="mt-2 text-xs text-gray-500">
                <span className="font-semibold text-amber-400">{fmt(overtimePay)}</span> of that is
